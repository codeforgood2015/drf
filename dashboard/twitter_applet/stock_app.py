...
tickers = [splitext(x)[0].split("table_")[-1] for x in tickers] #List of names of stocks ['AAPL', 'GOOG',...]

def get_ticker_data(ticker):
    print 'get_ticker_data'
    fname = join(data_dir, "table_%s.csv" % ticker.lower()) # daily/table_goog.csv
    data = pd.read_csv(
        fname,
    ...
    data = pd.DataFrame({ticker : data.c, ticker + "_returns" : data.c.diff()}) #Two columns: Ticker (AAPL) and returns (AAPL_returns)
    return data

#Cache of data
pd_cache = {}

# Gets data from both tickers and concats into 1 df
...
class StockApp(VBox):
    ...
    def set_children(self):
        ...
        self.histrow.children = [self.hist1]
